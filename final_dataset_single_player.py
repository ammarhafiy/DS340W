...
def find_first_existing(df, possible_cols):
    """
    Return the first column name that exists in df from a list of possible names.
    """
...
print(opponent_stats.columns.tolist())
...
fg3_pct_col = find_first_existing(opponent_stats, ["FG3_PCT", "OPP_FG3_PCT", "FG3A_PCT"])
...
# -----------------------------
# Step 3: Merge player and opponent data
# -----------------------------
...
# -----------------------------
merged_df["SeasonAvgPTS"] = merged_df["PTS"].mean()
merged_df["SeasonAvgREB"] = merged_df["REB"].mean()
merged_df["SeasonAvgAST"] = merged_df["AST"].mean()
merged_df["SeasonAvgTOV"] = merged_df["TOV"].mean()
merged_df["SeasonAvgFG_PCT"] = merged_df["FG_PCT"].mean()
